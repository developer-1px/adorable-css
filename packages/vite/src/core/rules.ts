...
export const RULES: Rules = {
	...
	underline: (value: string) => `text-decoration:underline ${makeValues(value)};`,
	...
	bxc: (value: string) =>
		`border-left-color:${makeColor(value)};border-right-color:${makeColor(value)};`,
	byc: (value: string) =>
		`border-top-color:${makeColor(value)};border-bottom-color:${makeColor(value)};`,
	...
	rt: (value: string) =>
		`border-top-left-radius:${rpx(value)};border-top-right-radius:${rpx(value)};`,
	rr: (value: string) =>
		`border-top-right-radius:${rpx(value)};border-bottom-right-radius:${rpx(value)};`,
	rb: (value: string) =>
		`border-bottom-left-radius:${rpx(value)};border-bottom-right-radius:${rpx(value)};`,
	rl: (value: string) =>
		`border-top-left-radius:${rpx(value)};border-bottom-left-radius:${rpx(value)};`,
	...
	contain: () =>
		`background-size:contain;background-position:center;background-repeat:no-repeat;object-fit:contain;`,
	cover: () =>
		`background-size:cover;background-position:center;background-repeat:no-repeat;object-fit:cover;`,
	...
	'nowrap...': () =>
		`white-space:nowrap;text-overflow:ellipsis;overflow:hidden;flex-shrink:1;max-width:100%;`,
	...
	'line-clamp': (value: string) =>
		`display:-webkit-box;-webkit-line-clamp:${value};-webkit-box-orient:vertical;overflow:hidden;`,
	...
	vbox: (value = '') =>
		`&{display:flex;flex-flow:column;${makeVBoxWithSemi(value)}}${makeVBoxFill()}`,
	wrap: (value = '') =>
		`&{display:flex;flex-flow:wrap;${makeHBoxWithSemi(value)}}${makeHBoxFill()}`,
	pack: () => `&{display:flex;align-items:center;justify-content:center;}${makeHBoxFill()}`,
	hpack: () =>
		`&{display:flex;flex-flow:row;align-items:center;justify-content:center;}${makeHBoxFill()}`,
	vpack: () =>
		`&{display:flex;flex-flow:column;align-items:center;justify-content:center;}${makeVBoxFill()}`,
	'hbox(': () => ``,
	'vbox(': () => ``,
	...
	'flex-wrap-reverse': () =>
		'&{flex-wrap:wrap-reverse;}:where(&>*){max-width:100%;max-height:100%;}',
	...
	layer: (value = '') => {
		...
		return (
			`position:absolute;` +
			Object.keys(pos)
				.map((value: string) => `${value}:${px(pos[value])};`)
				.join('')
		);
	},
	...
	blind: () =>
		`position:absolute;width:1px;height:1px;padding:0;border:0;margin:-1px;white-space:nowrap;overflow:hidden;clip-path:inset(100%);`,
	'sr-only': () =>
		`position:absolute;width:1px;height:1px;padding:0;border:0;margin:-1px;white-space:nowrap;overflow:hidden;clip-path:inset(100%);`,
	gone: () =>
		`position:absolute;width:1px;height:1px;padding:0;border:0;margin:-1px;white-space:nowrap;overflow:hidden;clip-path:inset(100%);`,
	...
	'user-select': (value: string) =>
		`user-select:${cssvar(value)};-webkit-user-select:${cssvar(value)};`,
