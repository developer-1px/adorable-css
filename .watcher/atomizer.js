  ...
  value
    ...
    .map((value, index) => {
      ...
      switch (index) {
        ...
        case 2: {
          return `letter-spacing: ${px(percentToEm(value))}`;
        }
      }
    })
    .join(";");
...
const makeColor = (value = "transparent") => {
  ...
  if (a && rgb.length === 4)
    return (
      "rgba(" +
      [...rgb.slice(1)].map((value) => parseInt(value, 16)).join(",") +
      ",." +
      a +
      ")"
    );
...
const RULES = {
  ...
  pack: (value) => `display:flex; align-items:center; justify-content:center`,
  flex: (value) => `flex:1`,
...
const PREFIX_RULES = {
  ...
  "mobile:": (value) =>
    `@media only screen and (max-width: 767px) { html & { ${value} }}`,
};
...
const generateCss = (classList) => {
  ...
  classList.forEach((cls) => {
    ...
    const styleText = result.includes("&")
      ? result.replace(/&/g, escapedClass)
      : `${escapedClass}{${result}${important}}`;
